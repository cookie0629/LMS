...
**最新进展**（2025-11-27 嵌入封面提取）：
- ✅ 音频解析阶段新增嵌入式封面提取，支持 MP3 ID3v2、FLAC Picture、MP4 covr
- ✅ `audio::ImageReader` 扩展 MIME 类型与数据缓存能力，供后续 Artwork 服务使用
- ✅ README、开发日志、学习进度文件同步说明新能力

**最新进展**（2025-11-27 `.lmsignore` 支持）：
- ✅ 扫描器支持 `.lmsignore` 文件，目录内放置该文件即可整体跳过扫描
- ✅ `ScannerSettings` 暴露常量，`discoverFiles()` 提供可选忽略文件参数
- ✅ README 与学习进度文件补充使用说明

**最新进展**（2025-11-27 艺术家白名单）：
- ✅ `TrackMetadataParser` 支持 `artists-to-not-split` 白名单，避免拆分如 “AC/DC” 的合法名称
- ✅ 构造函数归一化分隔符与白名单，按长度优先匹配防止部分覆盖
- ✅ `ScannerService` 可从配置读取 `scanner.metadata.artists-to-not-split`，README/文档同步

**最新进展**（2025-11-28 嵌入封面入库）：
- ✅ 新增 `TrackEmbeddedImage` 数据对象及 ID 类型，自动映射到 `track_embedded_image` 表
- ✅ `AudioFileScanOperation` 在扫描阶段将解析到的嵌入式封面写入该表，缺失时自动清理旧数据
- ✅ README / 开发日志 / 学习进度同步说明“提取并持久化”能力，为后续 Artwork Service 做准备
- ✅ 引入 `ImageType` / `TrackEmbeddedImageLink`，统一记录封面类型与索引，后续可扩展多张封面

**文件**：
- `src/libs/database/include/database/objects/TrackEmbeddedImageId.hpp`
- `src/libs/database/include/database/objects/TrackEmbeddedImage.hpp`
- `src/libs/database/impl/objects/TrackEmbeddedImage.cpp`
- `src/libs/database/include/database/objects/TrackEmbeddedImageLinkId.hpp`
- `src/libs/database/include/database/objects/TrackEmbeddedImageLink.hpp`
- `src/libs/database/impl/objects/TrackEmbeddedImageLink.cpp`
- `src/libs/database/impl/Session.cpp`
- `src/libs/database/CMakeLists.txt`
- `src/libs/services/scanner/impl/scanners/audiofile/AudioFileScanOperation.cpp`
- `README.md`
- `学习进度跟踪.md`

**文件**：
- `src/libs/audio/include/audio/IImageReader.hpp`
- `src/libs/audio/impl/AudioFileInfo.cpp`
- `README.md`
- `学习进度跟踪.md`
...
**最新进展**（2025-11-28 Artwork Service 基础实现）：
- ✅ 创建 Image 库：实现 `IEncodedImage` 接口和 `EncodedImage` 类，支持从文件或数据构造编码图像
- ✅ 实现 Artwork 数据库对象：`ArtworkId`、`Artwork` 类，支持从 `TrackEmbeddedImage` 获取封面
- ✅ 实现简化版 Artwork Service：`IArtworkService` 接口和 `ArtworkService` 实现
  - 支持从 `ArtworkId` 获取图像（通过 `TrackEmbeddedImage`）
  - 支持默认专辑封面和艺术家图像（从 SVG 文件加载）
  - 简化版：暂不支持图像调整大小和缓存功能
- ✅ 更新 CMake 配置：添加 `lmsimage` 和 `lmsartwork` 库
- ✅ 文档同步：README 和开发日志更新

**文件**：
- `src/libs/image/include/image/IEncodedImage.hpp`
- `src/libs/image/include/image/EncodedImage.hpp`
- `src/libs/image/impl/EncodedImage.cpp`
- `src/libs/image/CMakeLists.txt`
- `src/libs/database/include/database/objects/ArtworkId.hpp`
- `src/libs/database/include/database/objects/Artwork.hpp`
- `src/libs/database/impl/objects/Artwork.cpp`
- `src/libs/services/artwork/include/services/artwork/IArtworkService.hpp`
- `src/libs/services/artwork/impl/ArtworkService.hpp/cpp`
...
## 最新进展（2025-11-29 TrackArtistLink 实现）

### TrackArtistLink 数据库对象 ✅

实现了 Track 和 Artist 之间的多对多关联，支持多种关联类型。

**实现内容**：
- ✅ 在 `Types.hpp` 中添加 `TrackArtistLinkType` 枚举，支持 11 种关联类型（Artist, Arranger, Composer, Conductor, Lyricist, Mixer, Performer, Producer, ReleaseArtist, Remixer, Writer）
- ✅ 创建 `TrackArtistLinkId` 类型
- ✅ 实现 `TrackArtistLink` 数据库对象，包含：
  - 关联类型（type）
  - 子类型（subtype）
  - 艺术家名称和排序名称（artist_name, artist_sort_name）
  - MBID 匹配标志（artist_mbid_matched）
  - Track 和 Artist 的外键关联
- ✅ 实现 `TrackArtistLink::create()` 方法，支持创建关联并避免重复
- ✅ 实现 `TrackArtistLink::removeForTrack()` 方法，用于清理旧的关联
- ✅ 在 `Session` 中映射 `TrackArtistLink` 到 `track_artist_link` 表
- ✅ 更新 `AudioFileScanOperation`，在扫描时自动创建 TrackArtistLink：
  - 为 `metadata.artists` 创建 `TrackArtistLinkType::Artist` 关联
  - 为 `metadata.albumArtists` 创建 `TrackArtistLinkType::ReleaseArtist` 关联
  - 支持单个 `artist` 和 `albumArtist` 字段的回退

**相关文件**：
- `src/libs/database/include/database/Types.hpp`（添加 TrackArtistLinkType 枚举）
- `src/libs/database/include/database/objects/TrackArtistLinkId.hpp`（新建）
- `src/libs/database/include/database/objects/TrackArtistLink.hpp`（新建）
- `src/libs/database/impl/objects/TrackArtistLink.cpp`（新建）
- `src/libs/database/CMakeLists.txt`（添加 TrackArtistLink.cpp）
- `src/libs/database/impl/Session.cpp`（映射 TrackArtistLink）
- `src/libs/services/scanner/impl/scanners/audiofile/AudioFileScanOperation.cpp`（创建 TrackArtistLink 关联）
- `README.md`（更新功能说明）

**技术细节**：
- TrackArtistLink 使用 Wt::Dbo 的 `belongsTo` 关系，支持级联删除
- 在创建新关联前，先删除该 Track 的所有旧关联，确保数据一致性
- 支持艺术家名称和排序名称的存储，便于后续查询和显示
- 关联类型使用枚举，类型安全且易于扩展

**下一步**：
- 支持更多关联类型（如 Composer, Lyricist 等）的元数据提取
- 实现 TrackArtistLink 的查询方法（按类型、按艺术家等）
- 优化关联创建逻辑，避免不必要的删除和重建

**相关模块**：database, scanner

---

## 最新进展（2025-11-29 ImageCache 实现）

### ImageCache 图像缓存系统 ✅

实现了 ArtworkService 的图像缓存功能，提高图像加载性能。

**实现内容**：
- ✅ 创建 `ImageCache` 类，支持基于内存大小的缓存管理
- ✅ 实现线程安全的缓存操作（使用 `std::shared_mutex`）
- ✅ 实现 LRU 风格的缓存淘汰策略（当缓存满时随机删除条目）
- ✅ 缓存统计：支持缓存命中/未命中计数
- ✅ 更新 `ArtworkService` 以使用 `ImageCache`：
  - 在 `getImage()` 中先检查缓存
  - 从数据库获取图像后自动添加到缓存
  - 支持 `flushCache()` 清空缓存
- ✅ 更新 `IdType` 宏，添加 `std::hash` 特化支持，使所有 ID 类型可用于哈希容器

**相关文件**：
- `src/libs/services/artwork/impl/ImageCache.hpp`（新建）
- `src/libs/services/artwork/impl/ImageCache.cpp`（新建）
- `src/libs/services/artwork/impl/ArtworkService.hpp`（添加 ImageCache 成员）
- `src/libs/services/artwork/impl/ArtworkService.cpp`（集成 ImageCache）
- `src/libs/services/artwork/CMakeLists.txt`（添加 ImageCache.cpp）
- `src/libs/database/include/database/IdType.hpp`（添加 std::hash 特化）
- `README.md`（更新功能说明）

**技术细节**：
- ImageCache 使用 `std::unordered_map` 存储缓存条目
- 使用 `std::shared_mutex` 实现读写锁，支持并发读取
- 缓存大小限制为 50MB（可配置）
- 当缓存满时，使用随机删除策略（`core::random::pickRandom`）释放空间
- 缓存键使用 `ArtworkId`，简化版暂不支持大小调整
