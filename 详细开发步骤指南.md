# LMS 项目详细开发步骤指南

## 📋 目录

1. [开发环境准备](#1-开发环境准备)
2. [项目结构理解](#2-项目结构理解)
3. [代码风格和规范](#3-代码风格和规范)
4. [开发工作流程](#4-开发工作流程)
5. [构建和编译](#5-构建和编译)
6. [测试](#6-测试)
7. [调试](#7-调试)
8. [代码提交](#8-代码提交)
9. [常见开发任务](#9-常见开发任务)

---

## 1. 开发环境准备

### 1.1 必需工具安装

#### Windows + WSL2 环境

```bash
# 1. 安装 WSL2（在 PowerShell 管理员模式）
wsl --install

# 2. 进入 WSL2
wsl

# 3. 更新系统
sudo apt update && sudo apt upgrade -y
```

#### 安装开发工具链

```bash
# 基础编译工具
sudo apt install -y \
  build-essential \
  cmake \
  git \
  pkg-config \
  ninja-build

# C++ 编译器（确保支持 C++20）
sudo apt install -y \
  g++-12 \
  clang-14

# 代码格式化工具
sudo apt install -y \
  clang-format \
  clang-tidy

# 调试工具
sudo apt install -y \
  gdb \
  valgrind
```

#### 安装项目依赖

```bash
# Boost 库（完整安装）
sudo apt install -y libboost-all-dev

# FFmpeg 开发库
sudo apt install -y \
  libavcodec-dev \
  libavformat-dev \
  libavutil-dev \
  libavfilter-dev \
  libswscale-dev \
  libswresample-dev \
  ffmpeg

# 其他依赖
sudo apt install -y \
  libstb-dev \
  libconfig++-dev \
  libtag1-dev \
  libpam0g-dev \
  libpugixml-dev \
  libgtest-dev \
  libarchive-dev \
  libxxhash-dev \
  libssl-dev \
  libsqlite3-dev
```

#### 安装 Wt 框架

```bash
cd ~
wget https://github.com/emweb/wt/archive/4.12.1.tar.gz
tar -xzf 4.12.1.tar.gz
cd wt-4.12.1

mkdir build && cd build
cmake .. \
  -DCMAKE_BUILD_TYPE=Release \
  -DCMAKE_INSTALL_PREFIX=/usr/local \
  -DSHARED_LIBS=ON

make -j$(nproc)
sudo make install
sudo ldconfig
```

### 1.2 配置开发工具

#### VS Code 配置（推荐）

创建 `.vscode/settings.json`：

```json
{
  "C_Cpp.default.cppStandard": "c++20",
  "C_Cpp.default.compilerPath": "/usr/bin/g++",
  "C_Cpp.default.includePath": [
    "${workspaceFolder}/**",
    "/usr/local/include",
    "/usr/include"
  ],
  "files.associations": {
    "*.hpp": "cpp",
    "*.cpp": "cpp"
  },
  "editor.formatOnSave": true,
  "editor.defaultFormatter": "ms-vscode.cpptools",
  "[cpp]": {
    "editor.defaultFormatter": "ms-vscode.cpptools",
    "editor.formatOnSave": true
  },
  "C_Cpp.clang_format_style": "file",
  "C_Cpp.clang_format_path": "/usr/bin/clang-format"
}
```

创建 `.vscode/launch.json`（调试配置）：

```json
{
  "version": "0.2.0",
  "configurations": [
    {
      "name": "Debug LMS",
      "type": "cppdbg",
      "request": "launch",
      "program": "${workspaceFolder}/build/src/lms/lms",
      "args": ["${workspaceFolder}/conf/lms.conf"],
      "stopAtEntry": false,
      "cwd": "${workspaceFolder}",
      "environment": [],
      "externalConsole": false,
      "MIMode": "gdb",
      "setupCommands": [
        {
          "description": "Enable pretty-printing for gdb",
          "text": "-enable-pretty-printing",
          "ignoreFailures": true
        }
      ],
      "preLaunchTask": "build"
    }
  ]
}
```

创建 `.vscode/tasks.json`（构建任务）：

```json
{
  "version": "2.0.0",
  "tasks": [
    {
      "label": "build",
      "type": "shell",
      "command": "cmake --build build --config Debug -j$(nproc)",
      "group": {
        "kind": "build",
        "isDefault": true
      },
      "problemMatcher": ["$gcc"]
    },
    {
      "label": "clean",
      "type": "shell",
      "command": "cmake --build build --target clean"
    }
  ]
}
```

---

## 2. 项目结构理解

### 2.1 目录结构详解

```
lms/
├── src/                    # 源代码目录
│   ├── libs/              # 核心库
│   │   ├── audio/         # 音频处理库
│   │   │   ├── include/   # 公共接口（.hpp）
│   │   │   └── impl/      # 实现文件（.cpp, .hpp）
│   │   ├── core/          # 核心工具库
│   │   ├── database/      # 数据库层
│   │   ├── image/         # 图像处理
│   │   ├── services/      # 业务服务
│   │   │   ├── scanner/   # 扫描服务
│   │   │   ├── auth/      # 认证服务
│   │   │   ├── transcoding/ # 转码服务
│   │   │   └── ...
│   │   ├── som/           # 自组织映射
│   │   └── subsonic/      # Subsonic API
│   ├── lms/               # 主程序
│   │   ├── main.cpp       # 程序入口
│   │   └── ui/            # Web UI 代码
│   └── tools/             # 工具程序
├── approot/                # Web UI 模板（XML）
├── docroot/                # 静态资源（CSS, JS）
├── conf/                   # 配置文件
├── cmake/                  # CMake 模块
└── build/                  # 构建目录（gitignore）
```

### 2.2 代码组织原则

#### 接口和实现分离

- **include/**：公共接口（.hpp），供其他模块使用
- **impl/**：实现文件（.cpp, .hpp），内部实现细节

**示例**：
```
libs/audio/
├── include/audio/
│   └── IAudioFileInfo.hpp  # 接口定义
└── impl/
    └── ParseAudioFileInfo.cpp  # 实现
```

#### 命名规范

- **接口类**：以 `I` 开头，如 `IAudioFileInfo`
- **实现类**：具体名称，如 `AudioFileInfo`
- **文件命名**：与类名一致，使用 PascalCase

### 2.3 CMake 结构

#### 主 CMakeLists.txt

```cmake
# 设置 C++20 标准
set(CMAKE_CXX_STANDARD 20)

# 添加子目录
add_subdirectory(src)
```

#### 库的 CMakeLists.txt 示例

```cmake
# 创建库
add_library(lmsaudio
    impl/ParseAudioFileInfo.cpp
    ...
)

# 设置包含目录
target_include_directories(lmsaudio PUBLIC
    include/
)

# 链接依赖
target_link_libraries(lmsaudio PUBLIC
    TagLib::TagLib
    ...
)
```

---

## 3. 代码风格和规范

### 3.1 代码格式化

项目使用 `.clang-format` 定义代码风格。

#### 格式化命令

```bash
# 格式化单个文件
clang-format -i src/lms/main.cpp

# 格式化整个项目
find src -name "*.cpp" -o -name "*.hpp" | xargs clang-format -i

# 检查格式（不修改）
clang-format --dry-run --Werror src/lms/main.cpp
```

#### 主要风格规则

- **基于 Microsoft 风格**
- **C++20 标准**
- **指针对齐**：左对齐（`Type* ptr`）
- **命名空间缩进**：全部缩进
- **列宽限制**：无限制（ColumnLimit: 0）

### 3.2 代码检查

项目使用 `.clang-tidy` 进行静态分析。

#### 运行 clang-tidy

```bash
# 检查单个文件
clang-tidy src/lms/main.cpp -- -I./src -I/usr/local/include

# 检查整个项目（需要 compile_commands.json）
# 首先生成 compile_commands.json
cd build
cmake .. -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

# 然后运行 clang-tidy
clang-tidy -p build src/**/*.cpp
```

#### 主要检查规则

- **bugprone-***：潜在 bug
- **cppcoreguidelines-***：C++ 核心指南
- **performance-***：性能问题
- **readability-***：可读性

### 3.3 代码规范要点

#### 头文件保护

```cpp
#pragma once

// 或传统方式
#ifndef LMS_CORE_LOGGER_HPP
#define LMS_CORE_LOGGER_HPP
// ...
#endif
```

#### 命名空间

```cpp
namespace lms::core
{
    class Logger
    {
        // ...
    };
}
```

#### 智能指针

优先使用智能指针：

```cpp
std::unique_ptr<IAudioFileInfo> parseAudioFile(...);
std::shared_ptr<IScannerService> scanner;
```

#### 异常处理

```cpp
try
{
    // 代码
}
catch (const std::exception& e)
{
    LMS_LOG(MAIN, ERROR, "Error: " << e.what());
    // 处理
}
```

---

## 4. 开发工作流程

### 4.1 初始化项目

```bash
# 1. 克隆项目（如果还没有）
git clone https://github.com/epoupon/lms.git
cd lms

# 2. 创建构建目录
mkdir build
cd build

# 3. 配置 CMake（Debug 模式用于开发）
cmake .. \
  -DCMAKE_BUILD_TYPE=Debug \
  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

# 4. 编译
make -j$(nproc)

# 5. 运行测试
ctest
```

### 4.2 日常开发流程

#### 步骤 1：创建功能分支

```bash
# 从主分支创建新分支
git checkout -b feature/my-new-feature

# 或从 develop 分支
git checkout develop
git pull
git checkout -b feature/my-new-feature
```

#### 步骤 2：编写代码

1. **创建新文件**：
   - 接口文件放在 `include/` 目录
   - 实现文件放在 `impl/` 目录
   - 更新相应的 `CMakeLists.txt`

2. **修改现有代码**：
   - 遵循现有代码风格
   - 添加必要的注释
   - 更新相关文档

#### 步骤 3：格式化代码

```bash
# 格式化修改的文件
clang-format -i src/path/to/file.cpp

# 或使用 VS Code 的格式化功能（Ctrl+Shift+I）
```

#### 步骤 4：编译和测试

```bash
cd build
make -j$(nproc)

# 运行测试
ctest

# 或运行特定测试
ctest -R TestName
```

#### 步骤 5：调试（如有问题）

见 [第 7 节：调试](#7-调试)

#### 步骤 6：提交代码

```bash
# 1. 查看修改
git status
git diff

# 2. 添加文件
git add src/path/to/file.cpp

# 3. 提交（使用清晰的提交信息）
git commit -m "feat: 添加新功能描述

详细说明：
- 做了什么
- 为什么这样做
- 如何测试"
```

### 4.3 Git 工作流建议

#### 提交信息格式

```
<type>: <subject>

<body>

<footer>
```

**类型（type）**：
- `feat`: 新功能
- `fix`: 修复 bug
- `docs`: 文档更新
- `style`: 代码格式（不影响功能）
- `refactor`: 重构
- `test`: 测试相关
- `chore`: 构建/工具相关

**示例**：
```
feat: 添加音频文件格式检测功能

实现了基于文件扩展名的格式检测，支持 MP3、FLAC、OGG 等格式。

测试方法：
1. 运行测试套件
2. 使用测试音频文件验证

Closes #123
```

---

## 5. 构建和编译

### 5.1 基本构建命令

```bash
# 配置（首次或修改 CMakeLists.txt 后）
cd build
cmake .. -DCMAKE_BUILD_TYPE=Debug

# 编译
make -j$(nproc)

# 清理
make clean

# 重新编译
make -j$(nproc)
```

### 5.2 构建选项

#### Debug 模式（开发）

```bash
cmake .. -DCMAKE_BUILD_TYPE=Debug
```

特点：
- 包含调试符号
- 优化关闭
- 断言启用

#### Release 模式（生产）

```bash
cmake .. -DCMAKE_BUILD_TYPE=Release
```

特点：
- 优化开启
- 调试符号可能被剥离
- 性能最佳

#### 其他选项

```bash
# 启用测试
cmake .. -DENABLE_TESTS=ON

# 启用基准测试
cmake .. -DBUILD_BENCHMARKS=ON

# 导出编译命令（用于 clang-tidy）
cmake .. -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
```

### 5.3 增量编译

```bash
# 只编译修改的文件
make -j$(nproc)

# 只编译特定目标
make lms

# 查看编译依赖
make depend
```

### 5.4 常见编译问题

#### 问题 1：找不到头文件

```bash
# 检查包含路径
cmake .. --debug-output 2>&1 | grep include

# 手动添加包含路径（在 CMakeLists.txt 中）
target_include_directories(target_name PRIVATE
    /path/to/include
)
```

#### 问题 2：链接错误

```bash
# 检查库路径
ldconfig -p | grep library_name

# 在 CMakeLists.txt 中添加库路径
target_link_directories(target_name PRIVATE
    /path/to/lib
)
```

---

## 6. 测试

### 6.1 运行测试

```bash
# 运行所有测试
cd build
ctest

# 详细输出
ctest --verbose

# 运行特定测试
ctest -R TestName

# 并行运行
ctest -j$(nproc)
```

### 6.2 编写测试

#### 测试文件位置

- 单元测试：`src/libs/*/test/`
- 集成测试：`src/libs/*/test/`

#### 测试示例

```cpp
#include <gtest/gtest.h>
#include "core/Path.hpp"

TEST(PathTest, BasicOperations)
{
    lms::core::Path path{"/tmp/test"};
    
    EXPECT_TRUE(path.exists());
    EXPECT_FALSE(path.isDirectory());
}
```

#### 添加到 CMakeLists.txt

```cmake
# 在相应的库的 CMakeLists.txt 中
if(ENABLE_TESTS)
    add_executable(test_myfeature
        test/MyFeatureTest.cpp
    )
    
    target_link_libraries(test_myfeature PRIVATE
        lmscore
        GTest::gtest
        GTest::gtest_main
    )
    
    add_test(NAME MyFeatureTest COMMAND test_myfeature)
endif()
```

### 6.3 测试最佳实践

1. **测试命名**：清晰描述测试内容
2. **测试隔离**：每个测试独立
3. **测试覆盖**：覆盖主要功能点
4. **边界测试**：测试边界情况

---

## 7. 调试

### 7.1 使用 GDB

#### 基本调试

```bash
# 启动调试
gdb ./build/src/lms/lms

# 设置参数
(gdb) set args conf/lms.conf

# 设置断点
(gdb) break main
(gdb) break src/lms/main.cpp:100

# 运行
(gdb) run

# 单步执行
(gdb) step      # 进入函数
(gdb) next      # 下一行
(gdb) continue  # 继续执行

# 查看变量
(gdb) print variable_name
(gdb) print *pointer

# 查看调用栈
(gdb) backtrace
(gdb) frame 1

# 退出
(gdb) quit
```

#### 常用 GDB 命令

```bash
# 列出代码
(gdb) list
(gdb) list 50,100

# 查看变量类型
(gdb) ptype variable_name

# 监视变量
(gdb) watch variable_name

# 条件断点
(gdb) break file.cpp:100 if condition

# 命令历史
(gdb) show commands
```

### 7.2 使用 VS Code 调试

1. 打开 `.vscode/launch.json`
2. 设置断点（点击行号左侧）
3. 按 F5 开始调试
4. 使用调试工具栏：
   - 继续（F5）
   - 单步跳过（F10）
   - 单步进入（F11）
   - 单步跳出（Shift+F11）

### 7.3 日志调试

#### 使用日志系统

```cpp
#include "core/ILogger.hpp"

LMS_LOG(MODULE_NAME, INFO, "信息消息");
LMS_LOG(MODULE_NAME, DEBUG, "调试信息: " << variable);
LMS_LOG(MODULE_NAME, ERROR, "错误: " << error_message);
```

#### 配置日志级别

在 `conf/lms.conf` 中：

```
log-min-severity = "debug";  # 显示所有日志
log-file = "/var/lms/lms.log";  # 输出到文件
```

### 7.4 Valgrind（内存检查）

```bash
# 检查内存泄漏
valgrind --leak-check=full ./build/src/lms/lms conf/lms.conf

# 检查未初始化内存
valgrind --track-origins=yes ./build/src/lms/lms conf/lms.conf
```

---

## 8. 代码提交

### 8.1 提交前检查清单

- [ ] 代码能够编译
- [ ] 所有测试通过
- [ ] 代码已格式化（clang-format）
- [ ] 代码检查通过（clang-tidy）
- [ ] 提交信息清晰
- [ ] 没有临时文件或调试代码

### 8.2 提交流程

```bash
# 1. 查看状态
git status

# 2. 查看差异
git diff

# 3. 添加文件
git add src/path/to/file.cpp

# 4. 提交
git commit -m "feat: 功能描述"

# 5. 推送到远程（如果已设置）
git push origin feature/my-new-feature
```

### 8.3 Pull Request 流程

1. **创建 PR**：
   - 在 GitHub 上创建 Pull Request
   - 从功能分支到 `develop` 分支

2. **PR 描述**：
   - 说明做了什么
   - 为什么这样做
   - 如何测试
   - 相关 Issue

3. **代码审查**：
   - 等待审查
   - 根据反馈修改

4. **合并**：
   - 审查通过后合并

---

## 9. 常见开发任务

### 9.1 添加新功能模块

#### 步骤 1：创建目录结构

```bash
mkdir -p src/libs/mymodule/include/mymodule
mkdir -p src/libs/mymodule/impl
```

#### 步骤 2：创建接口文件

`src/libs/mymodule/include/mymodule/IMyModule.hpp`:

```cpp
#pragma once

namespace lms::mymodule
{
    class IMyModule
    {
    public:
        virtual ~IMyModule() = default;
        virtual void doSomething() = 0;
    };

    std::unique_ptr<IMyModule> createMyModule();
}
```

#### 步骤 3：创建实现文件

`src/libs/mymodule/impl/MyModule.cpp`:

```cpp
#include "mymodule/IMyModule.hpp"

namespace lms::mymodule
{
    class MyModule : public IMyModule
    {
    public:
        void doSomething() override
        {
            // 实现
        }
    };

    std::unique_ptr<IMyModule> createMyModule()
    {
        return std::make_unique<MyModule>();
    }
}
```

#### 步骤 4：创建 CMakeLists.txt

`src/libs/mymodule/CMakeLists.txt`:

```cmake
add_library(lmsmymodule
    impl/MyModule.cpp
)

target_include_directories(lmsmymodule PUBLIC
    include/
)

target_link_libraries(lmsmymodule PUBLIC
    # 依赖库
)
```

#### 步骤 5：在主 CMakeLists.txt 中添加

`src/libs/CMakeLists.txt`:

```cmake
add_subdirectory(mymodule)
```

### 9.2 修改现有功能

1. **找到相关文件**：使用 grep 或代码搜索
2. **理解现有实现**：阅读代码和注释
3. **进行修改**：遵循现有代码风格
4. **测试**：运行相关测试
5. **更新文档**：如有必要

### 9.3 添加新的 API 端点

#### 位置

`src/libs/subsonic/impl/endpoints/`

#### 步骤

1. 创建端点文件：`MyEndpoint.cpp` 和 `MyEndpoint.hpp`
2. 实现端点逻辑
3. 在 `SubsonicResource.cpp` 中注册端点

### 9.4 添加新的数据库对象

#### 位置

`src/libs/database/include/database/objects/`

#### 步骤

1. 创建对象头文件：`MyObject.hpp`
2. 创建实现文件：`src/libs/database/impl/objects/MyObject.cpp`
3. 在 `Session.cpp` 中注册映射
4. 创建迁移脚本（如需要）

---

## 10. 开发技巧和最佳实践

### 10.1 代码搜索

```bash
# 搜索函数定义
grep -r "function_name" src/

# 搜索包含特定头文件
grep -r "#include.*header" src/

# 使用 ripgrep（更快）
rg "pattern" src/
```

### 10.2 理解代码流程

1. **从入口开始**：`src/lms/main.cpp`
2. **跟踪调用链**：使用 IDE 的"转到定义"
3. **阅读接口**：先看接口，再看实现
4. **查看测试**：测试代码是最好的文档

### 10.3 性能优化

1. **使用性能分析工具**：
   ```bash
   perf record ./build/src/lms/lms
   perf report
   ```

2. **数据库查询优化**：
   - 使用索引
   - 避免 N+1 查询
   - 使用 EXPLAIN QUERY PLAN

3. **内存管理**：
   - 使用智能指针
   - 避免不必要的拷贝
   - 使用移动语义

### 10.4 文档编写

- **代码注释**：解释"为什么"，而不是"是什么"
- **函数文档**：说明参数、返回值、异常
- **README 更新**：如有重大变更

---

## 11. 故障排除

### 11.1 编译错误

1. **检查依赖**：确保所有依赖已安装
2. **清理构建**：`rm -rf build && mkdir build`
3. **查看详细输出**：`make VERBOSE=1`

### 11.2 运行时错误

1. **查看日志**：检查日志输出
2. **使用调试器**：GDB 或 VS Code
3. **检查配置**：验证配置文件

### 11.3 测试失败

1. **单独运行测试**：`ctest -R TestName -V`
2. **检查测试数据**：确保测试数据存在
3. **查看测试输出**：检查错误信息

---

## 12. 资源链接

### 官方文档

- **Wt 框架**：https://www.webtoolkit.eu/wt/doc/
- **CMake**：https://cmake.org/documentation/
- **C++20**：https://en.cppreference.com/

### 项目相关

- **GitHub 仓库**：https://github.com/epoupon/lms
- **Issues**：https://github.com/epoupon/lms/issues
- **Discussions**：https://github.com/epoupon/lms/discussions

---

## ✅ 开发检查清单

开始开发前：
- [ ] 环境已配置
- [ ] 项目能够编译
- [ ] 测试能够运行
- [ ] IDE 已配置

开发过程中：
- [ ] 代码遵循风格规范
- [ ] 定期编译测试
- [ ] 添加必要的注释
- [ ] 编写或更新测试

提交前：
- [ ] 代码格式化
- [ ] 所有测试通过
- [ ] 提交信息清晰
- [ ] 没有调试代码

---

祝你开发顺利！💻🎵

