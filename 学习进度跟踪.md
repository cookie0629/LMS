...
- [x] 阶段 5：服务层 - Scanner Service（✅ 100% 完成）
  ...
  - [x] 完善 AudioFileScanOperation 的实际扫描逻辑（✅）
  - [x] 实现 ScanStepCheckForRemovedFiles（检查已删除的文件）（✅）
  - [x] 实现 ScanStepCheckForDuplicatedFiles（检查重复文件）（✅）
- [x] 实现 ScanStepUpdateLibraryFields（更新媒体库字段）（✅）
  - [x] 实现 ScanStepRemoveOrphanedDbEntries（删除孤立数据）（✅）
- [x] 实现 ScanStepOptimize（数据库优化）（✅）
- [x] 实现 ScanStepCompact（数据库压缩）（✅）
- [x] 增强 TrackMetadataParser（多艺术家拆分与标签容错，可配置）（✅）
- [x] 实现嵌入式封面提取（MP3/FLAC/MP4）（✅）
- [x] 持久化嵌入式封面到 TrackEmbeddedImage 表（✅）
- [x] 引入 ImageType / TrackEmbeddedImageLink（✅）
- [x] 支持艺术家白名单 `artists-to-not-split`（✅）
- [x] 支持 `.lmsignore` 目录忽略（✅）
- [x] 添加扫描步骤的错误处理与日志记录（✅）
- [x] 实现 TrackArtistLink 数据库对象（✅）
- [x] 支持多艺术家关联（Artist, ReleaseArtist 类型）（✅）
- [x] 在扫描时自动创建 TrackArtistLink 关联（✅）

- [x] 阶段 6：服务层 - Artwork Service（✅ 基础实现完成）
  - [x] 创建 Image 库（IEncodedImage, EncodedImage）（✅）
  - [x] 实现 Artwork 数据库对象（ArtworkId, Artwork）（✅）
  - [x] 实现简化版 Artwork Service（IArtworkService, ArtworkService）（✅）
  - [x] 支持从 TrackEmbeddedImage 读取封面图像（✅）
  - [x] 支持默认专辑封面和艺术家图像（✅）
  - [x] 实现 ImageCache 图像缓存系统（✅）
  - [x] 更新 IdType 宏添加 std::hash 支持（✅）
